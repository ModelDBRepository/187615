...
variants_matlab = {[7,1,1],[7,5,1],[8,1,1],[8,3,1]};
variant_chosen = 1;

geneNames = getgenenames();
titles = {'SCN1A [49]', 'SCN1A [52]', 'HCN1 [54]', 'HCN1 [56]'};
MT = getMT_kharche();
...
for iy=1:4
  for ix=1:length(variants)
    if iy < 3
      SPinset(iy,ix) = axes('position',[0.40+(ix-1)*0.17,0.79-(iy-1)*0.2,0.06,0.07]);
    end
    set(SP(iy,1+ix),'position', [0.31+(ix-1)*0.17, 0.7725-(iy-1)*0.2, 0.17, 0.16])
  end
...
if exist('hay/fig1_curves.mat')
  data_hay = load('hay/fig1_curves.mat');
end
if exist('almog/fig1_curves.mat')
  data_almog = load('almog/fig1_curves.mat');
end
...
for ivar=1:length(variants)
  ...
  subplot(SPinset(1,ivar)); a = bar(x_bar_control,0); set(a,'facecolor',col_control); axis([0,6,-10,10]); set(gca,'xtick',[],'fontsize',5); ylabel('%','rot',0);
  ...
  t = annotation(gcf,'textbox',[0.31+(ivar-1)*0.17,0.02,0.18,0.14]);set(t,'string',mystr,'fontsize',9,'linestyle','none');
